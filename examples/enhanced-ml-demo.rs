...
async fn demonstrate_enhanced_features() -> Result<()> {
    ...
    let findings = vec![
        ...
        .with_description(
            "This appears to be a hardcoded API key which poses a significant security risk"
                .to_string(),
        )
        .with_suggestion(
            "Move the API key to environment variables or a secure configuration system"
                .to_string(),
        ),
        // Performance finding
        Finding::new(
        ...
        .with_description(
            "This nested loop structure may cause performance issues with large datasets"
                .to_string(),
        )
        .with_suggestion(
            "Consider using HashMap for O(1) lookups or optimizing the algorithm".to_string(),
        ),
...
async fn demonstrate_adaptive_learning() -> Result<()> {
    ...
    println!(
        "   Early Stopped:        {}",
        if stats.early_stopped { "Yes ✋" } else { "No" }
    );
...
async fn compare_training_strategies() -> Result<()> {
    ...
    let time_improvement = if duration_adaptive < duration_fixed {
        ((duration_fixed.as_millis() - duration_adaptive.as_millis()) as f32
            / duration_fixed.as_millis() as f32)
            * 100.0
    } else {
        -((duration_adaptive.as_millis() - duration_fixed.as_millis()) as f32
            / duration_fixed.as_millis() as f32)
            * 100.0
    };
...
async fn demonstrate_real_world_usage() -> Result<()> {
    ...
    println!(
        "📚 Training on {} realistic examples...",
        training_data.len()
    );
