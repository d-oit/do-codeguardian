...
async fn main() -> Result<()> {
    ...
    for (i, finding) in findings.iter().enumerate() {
        ...
        let start = Instant::now();
        let enhanced_features = enhanced_extractor
        ...
        let start = Instant::now();
        let advanced_features = advanced_extractor
            .extract_advanced_features(finding)
            .await?;
        let advanced_time = start.elapsed();

        // Display results
        println!(
            "🔹 Basic Features ({} dims): {:.2}ms",
            basic_features.len(),
        ...
        println!(
            "🔹 Enhanced Features ({} dims): {:.2}ms",
            enhanced_features.len(),
    ...
    demonstrate_performance_comparison(
        &findings,
        &mut basic_extractor,
        &mut enhanced_extractor,
        &mut advanced_extractor,
    )
    .await?;
...
fn create_security_findings() -> Vec<Finding> {
    vec![
        ...
        .with_description(
            "User input is directly concatenated into SQL query without sanitization".to_string(),
        )
...
async fn demonstrate_performance_comparison(
    ...
    println!(
        "   • Per finding: {:.3}ms",
        basic_total.as_millis() as f64 / operations
    );
    println!(
        "   • Features/ms: {:.1}",
        operations / basic_total.as_millis() as f64
    );
    ...
    println!(
        "   • Per finding: {:.3}ms",
        enhanced_total.as_millis() as f64 / operations
    );
    println!(
        "   • Features/ms: {:.1}",
        operations / enhanced_total.as_millis() as f64
    );
    ...
    println!(
        "   • Per finding: {:.3}ms",
        advanced_total.as_millis() as f64 / operations
    );
    println!(
        "   • Features/ms: {:.1}",
        operations / advanced_total.as_millis() as f64
    );
