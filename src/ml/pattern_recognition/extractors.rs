...
impl CodeStructureExtractor {
    ...
    fn extract_code_metrics(&self, content: &str) -> Vec<f64> {
        ...
        for line in &lines {
            ...
            if trimmed.starts_with("class ") || trimmed.starts_with("struct ") || trimmed.starts_with("impl ") {
                class_count += 1.0;
            }
        ...
        let comment_ratio = if line_count > 0.0 { comment_lines / line_count } else { 0.0 };
        let avg_nesting = if line_count > 0.0 { nesting_depth / line_count } else { 0.0 };
...
impl FeatureExtractor for ConfigurationExtractor {
    fn extract_features(&self, content: &str, metadata: &HashMap<String, String>) -> Result<Vec<f64>> {
        let file_extension = metadata.get("file_extension").map(|s| s.as_str()).unwrap_or("");
...
impl ConfigurationExtractor {
    ...
    fn extract_json_features(&self, content: &str) -> Result<Vec<f64>> {
        ...
        let boolean_count = content.matches("true").count() as f64 + content.matches("false").count() as f64;
    ...
    fn extract_generic_config_features(&self, content: &str) -> Result<Vec<f64>> {
        ...
        let comment_lines = content.lines().filter(|line| line.trim().starts_with('#')).count() as f64;
...
impl_simple_extractor!(PerformancePatternExtractor, &[
    "loop_complexity",
    "memory_allocations",
    "io_operations",
    "database_queries",
...
impl_simple_extractor!(NamingPatternExtractor, &[
    "camel_case_usage",
    "snake_case_usage",
    "pascal_case_usage",
    "kebab_case_usage",
...
impl_simple_extractor!(ApiUsageExtractor, &[
    "api_call_count",
...
impl_simple_extractor!(DataFlowExtractor, &[
...
fn calculate_technical_density(content: &str) -> f64 {
    let technical_terms = [
        ...
        "encryption", "decryption", "serialization", "deserialization",
    ];
...
fn count_sql_injection_patterns(content: &str) -> f64 {
    ...
    patterns.iter().map(|p| content.matches(p).count()).sum::<usize>() as f64
}
...
fn count_error_handling(content: &str) -> f64 {
    let patterns = ["try", "catch", "except", "error", "exception", "Result", "Option"];
    patterns.iter().map(|p| content.matches(p).count()).sum::<usize>() as f64
}
