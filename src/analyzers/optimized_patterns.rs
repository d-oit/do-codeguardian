...
impl QualityPatterns {
    fn new() -> Self {
        Self {
            ...
            magic_numbers_fast: Regex::new(r"\b\d{2,}\b").unwrap(),

            // Simplified complex condition detection
            complex_conditions_fast: Regex::new(r"if\s*\([^)]*(?:&&||\|\|)[^)]*(?:&&|\|\|)[^)]*\)")
                .unwrap(),
            ...
            naming_violations_fast: Regex::new(r"\b[a-hk-wz]\s*=|fn\s+[A-Z]\w*|function\s+\w*_\w*")
                .unwrap(),
        }
    }
}
...
impl DependencyPatterns {
    fn new() -> Self {
        Self {
            ...
            package_json_deps: Regex::new(r#""([^"]+)":\s*"([^"]+)""#).unwrap(),

            // Cargo.toml dependency patterns
            cargo_toml_deps: Regex::new(r#"^(\w+)\s*=\s*(?:\{[^}]*\}|["'][^"']*["'])"#).unwrap(),

            // Version range detection
            version_ranges: Regex::new(r"[\*\^~]|>=|<=|\|\|").unwrap(),

            // Git dependency detection
            git_deps: Regex::new(r#"git\s*=\s*['\"][^'\"]*['\"]"#).unwrap(),
        }
    }
}
...
impl PatternCache {
    ...
    pub fn check_pattern(&mut self, pattern: &Regex, text: &str) -> bool {
        ...
        // Create a more efficient cache key using hash
        ...
        if self.cache.len() >= self.max_size {
            // Remove 25% of entries to avoid full cache clearing
            let to_remove = self.max_size / 4;
            let keys_to_remove: Vec<u64> = self.cache.keys().take(to_remove).cloned().collect();
...
mod tests {
    ...
    fn test_optimized_patterns() {
        ...
        assert!(SECURITY_PATTERNS
            .secrets_combined
            .is_match("api_key = \"sk-1234567890abcdef\""));
        assert!(PERFORMANCE_PATTERNS
            .nested_loops_fast
            .is_match("for (i=0; i<n; i++) { for (j=0; j<m; j++) {"));
        assert!(QUALITY_PATTERNS
            .magic_numbers_fast
            .is_match("timeout = 5000"));
    }
