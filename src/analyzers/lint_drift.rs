        ...
        match serde_json::from_str::<serde_json::Value>(content) {
            Ok(value) => {
                ...
                if content.trim() != formatted.trim() {
                    findings.push(
                        ...
                        .with_description(
                            "JSON formatting is inconsistent with standard pretty-printing"
                                .to_string(),
                        )
                        .with_suggestion(
                            "Run a JSON formatter to standardize formatting".to_string(),
                        ),
                    );
                }
            }
            Err(_) => {
                findings.push(
                    ...
                    .with_description(
                        "File appears to be JSON but contains syntax errors".to_string(),
                    )
        ...
        match serde_yaml::from_str::<serde_yaml::Value>(content) {
            Ok(_) => {
                ...
                if content.contains('\t') {
                    findings.push(
                        ...
                        .with_description(
                            "YAML specification requires spaces for indentation".to_string(),
                        )
                        .with_suggestion(
                            "Replace tabs with spaces (typically 2 spaces per indent level)"
                                .to_string(),
                        ),
                    );
                }
            ...
            Err(_) => {
                findings.push(
                    ...
                    .with_description(
                        "File appears to be YAML but contains syntax errors".to_string(),
                    )
