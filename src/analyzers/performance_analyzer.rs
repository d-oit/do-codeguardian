...
pub struct PerformanceAnalyzer {
    // Use lazy static patterns to avoid recompilation overhead
    nested_loop_pattern: &'static Regex,
    string_concat_pattern: &'static Regex,
    inefficient_collection_pattern: &'static Regex,
    blocking_io_pattern: &'static Regex,
    memory_leak_pattern: &'static Regex,
    inefficient_regex_pattern: &'static Regex,
}

// Lazy static regex patterns for optimal performance
static NESTED_LOOP_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"for\s*\([^{]*\{[^}]*for\s*\(|while\s*\([^{]*\{[^}]*while\s*\(|\.forEach\([^}]*\.forEach\("#).unwrap()
});

static STRING_CONCAT_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r#"\+\s*=\s*[\"']|String\s*\+|str\s*\+\s*str"#).unwrap());

static INEFFICIENT_COLLECTION_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"\.contains\(.*\).*for|\.indexOf\(.*\).*for|in\s+list.*for"#).unwrap()
});

static BLOCKING_IO_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"\.read\(\)|\.write\(\)|\.sleep\(|Thread\.sleep|time\.sleep|fs\.readFileSync|fs\.writeFileSync"#).unwrap()
});

static MEMORY_LEAK_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"setInterval\(|addEventListener\(.*,.*\)|on\(.*,.*function|\.on\(.*,.*=>"#)
        .unwrap()
});

static INEFFICIENT_REGEX_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"new\s+RegExp\(|Regex::new\(.*\).*for|Pattern\.compile\(.*\).*for"#).unwrap()
});
...
impl PerformanceAnalyzer {
    pub fn new() -> Self {
        Self {
            ...
            string_concat_pattern: &STRING_CONCAT_PATTERN,
            inefficient_collection_pattern: &INEFFICIENT_COLLECTION_PATTERN,
            blocking_io_pattern: &BLOCKING_IO_PATTERN,
            memory_leak_pattern: &MEMORY_LEAK_PATTERN,
            inefficient_regex_pattern: &INEFFICIENT_REGEX_PATTERN,
        }
    }
    ...
    fn analyze_performance_issues(&self, file_path: &Path, content: &[u8]) -> Result<Vec<Finding>> {
        ...
        for (line_num, line) in lines.iter().enumerate() {
            ...
            if self.string_concat_pattern.is_match(line)
                && self.is_in_loop_context(&lines, line_num)
            {
                findings.push(
                    Finding::new(
                        "performance",
            ...
            if self.inefficient_collection_pattern.is_match(line) {
                findings.push(
                    ...
                    .with_description(
                        "Linear search operations in loops can lead to O(n²) complexity"
                            .to_string(),
                    )
                    .with_suggestion(
                        "Consider using HashSet, HashMap, or other O(1) lookup data structures"
                            .to_string(),
                    ),
                );
            }
            ...
            if self.blocking_io_pattern.is_match(line)
                && !self.is_in_async_context(&lines, line_num)
            {
                let severity = if line.contains("sleep") {
                    Severity::High
                } else {
                    Severity::Medium
                };
            ...
            if self.inefficient_regex_pattern.is_match(line) {
                findings.push(
                    ...
                    .with_description(
                        "Compiling regex patterns repeatedly is expensive".to_string(),
                    )
                    .with_suggestion(
                        "Compile regex patterns once outside the loop and reuse them".to_string(),
                    ),
                );
            }
    ...
    fn check_language_specific_issues(
        ...
        if let Some(ext) = file_path.extension().and_then(|e| e.to_str()) {
            match ext {
                ...
                "js" | "ts" | "jsx" | "tsx" => {
                    findings.extend(self.check_javascript_performance(
                        file_path,
                        line,
                        line_number,
                    )?);
    ...
    fn check_rust_performance(
        ...
        if line.contains(".clone()") && !line.contains("// clone needed") {
            findings.push(
                ...
                .with_description(
                    "Cloning can be expensive; consider using references or borrowing".to_string(),
                )
                .with_suggestion(
                    "Use references (&) or borrowing instead of cloning when possible".to_string(),
                ),
            );
        }
        ...
        if line.contains("vec![]") && line.contains("push") {
            findings.push(
                ...
                .with_description(
                    "Growing vectors without capacity hints can cause multiple reallocations"
                        .to_string(),
                )
                .with_suggestion(
                    "Use Vec::with_capacity() if you know the approximate size".to_string(),
                ),
            );
        }
        ...
        if line.contains("String::new()") && line.contains("push_str") {
            findings.push(
                ...
                .with_description(
                    "Building strings character by character can be inefficient".to_string(),
                )
                .with_suggestion(
                    "Consider using format! macro or collecting into String".to_string(),
                ),
            );
        }
    ...
    fn check_javascript_performance(
        &self,
        file_path: &Path,
        line: &str,
        line_number: u32,
            ...
            && (line.contains("for") || line.contains("while"))
        {
            findings.push(
                ...
                .with_description(
                    "DOM queries are expensive and should be cached when used repeatedly"
                        .to_string(),
                )
        ...
        if line.contains(".filter(") && line.contains(".map(") {
            findings.push(
                ...
                .with_suggestion(
                    "Consider using reduce() or a single loop for better performance".to_string(),
                ),
            );
        }
        ...
        if line.contains(" == ") && !line.contains(" === ") {
            findings.push(
                ...
                .with_description(
                    "Loose equality (==) performs type coercion which can be slower".to_string(),
                )
                .with_suggestion(
                    "Use strict equality (===) for better performance and clarity".to_string(),
                ),
            );
        }
    ...
    fn check_python_performance(
        ...
        if line.contains("for ") && line.contains("append(") {
            findings.push(
                ...
                .with_description(
                    "List comprehensions are generally faster than append() in loops".to_string(),
                )
        ...
        if line.contains("str(") && line.contains(" + ") {
            findings.push(
                ...
                .with_description(
                    "String concatenation with + can be inefficient for multiple strings"
                        .to_string(),
                )
                .with_suggestion(
                    "Use f-strings, .join(), or .format() for better performance".to_string(),
                ),
            );
        }
    ...
    fn check_java_performance(
        ...
        if line.contains("String ") && line.contains(" + ") {
            findings.push(
                ...
                .with_description(
                    "String concatenation creates new objects and can be inefficient".to_string(),
                )
        ...
        if line.contains("new ArrayList()") {
            findings.push(
                ...
                .with_description(
                    "ArrayList without capacity hint may cause multiple resizing operations"
                        .to_string(),
                )
        ...
        for (line_num, line) in content.lines().enumerate() {
            if line.len() > MAX_LINE_LENGTH {
                findings.push(
                    ...
                    .with_description(
                        "Very long lines can impact readability and code review efficiency"
                            .to_string(),
                    )
                    .with_suggestion(
