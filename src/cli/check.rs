...
async fn apply_config_overrides(args: &mut CheckArgs, config: &mut Config, output_dir: &str) -> Result<()> {
...
async fn determine_files_to_scan(args: &CheckArgs, engine: &GuardianEngine) -> Result<Vec<PathBuf>> {
...
async fn handle_baseline_comparison(baseline_file: &Option<PathBuf>) -> Result<Option<crate::types::AnalysisResults>> {
...
async fn run_analysis_and_filter(args: &CheckArgs, _config: &Config, engine: &mut GuardianEngine, files_to_scan: &[PathBuf], ml_threshold: Option<f64>) -> Result<crate::types::AnalysisResults> {
    // Run analysis
...
async fn handle_output_and_reporting(args: &CheckArgs, results: &crate::types::AnalysisResults, config: &Config) -> Result<()> {
...
pub async fn run(mut args: CheckArgs, mut config: Config) -> Result<()> {
    ...
    let mut results = run_analysis_and_filter(&args, &config, &mut engine, &files_to_scan, ml_threshold).await?;
    ...
    handle_output_and_reporting(&args, &results, &config).await?;
