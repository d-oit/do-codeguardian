...
use crate::ml::production_validation::{
    ProductionValidationFramework, ValidationConfig, TestSuite, TestCase,
    TestCategory, TestCriticality, ExpectedPerformance, TestSuiteMetadata,
    ValidationStatus, DeploymentReadiness
};
use crate::ml::fann_classifier::FannClassifier;
use crate::types::{Finding, Severity, AnalysisResults};
use crate::Config;
use anyhow::Result;
...
pub async fn run(args: ModelValidationArgs, _config: &Config) -> Result<()> {
    ...
    println!("🔄 Starting validation for model: {}", args.model_path.display());
...
fn display_validation_results(result: &crate::ml::production_validation::ValidationResult) {
    ...
    println!("{} Overall Status: {:?}", status_icon, result.validation_status);
    
    // Key metrics
    println!("\n📈 Performance Metrics:");
    println!("  • Accuracy:    {:.3} ({:.1}%)", result.overall_metrics.accuracy, result.overall_metrics.accuracy * 100.0);
    println!("  • Precision:   {:.3} ({:.1}%)", result.overall_metrics.precision, result.overall_metrics.precision * 100.0);
    println!("  • Recall:      {:.3} ({:.1}%)", result.overall_metrics.recall, result.overall_metrics.recall * 100.0);
    println!("  • F1-Score:    {:.3} ({:.1}%)", result.overall_metrics.f1_score, result.overall_metrics.f1_score * 100.0);
    println!("  • FP Rate:     {:.3} ({:.1}%)", result.overall_metrics.false_positive_rate, result.overall_metrics.false_positive_rate * 100.0);
    
    println!("\n⚡ Performance:");
    println!("  • Avg Inference: {:.2}ms", result.overall_metrics.avg_inference_time_ms);
    println!("  • Max Inference: {:.2}ms", result.overall_metrics.max_inference_time_ms);
    println!("  • Min Inference: {:.2}ms", result.overall_metrics.min_inference_time_ms);
    
    // Validator breakdown
    println!("\n🔍 Validator Results:");
    for (validator_name, metrics) in &result.validator_results {
        let status = if metrics.accuracy >= 0.8 { "✅" } else { "❌" };
        println!("  {} {}: Accuracy {:.3}, F1 {:.3}", 
    ...
    if !result.test_suite_results.is_empty() {
        ...
        for (suite_name, suite_result) in &result.test_suite_results {
            ...
            let status = if passed_rate >= 0.9 { "✅" } else if passed_rate >= 0.7 { "⚠️" } else { "❌" };
            
            println!("  {} {}: {}/{} passed ({:.1}%)",
                    status, suite_name, suite_result.passed_tests,
                    suite_result.passed_tests + suite_result.failed_tests,
                    passed_rate * 100.0);
                    
            if suite_result.critical_failures.len() > 0 {
                println!("    ❗ Critical failures: {}", suite_result.critical_failures.len());
            }
        }
    }
    ...
    println!("  • Ready for Production: {}", 
            if result.deployment_readiness.ready_for_production { "✅ Yes" } else { "❌ No" });
    println!("  • Readiness Score: {:.1}/1.0", result.deployment_readiness.readiness_score);
...
fn print_performance_delta(metric_name: &str, delta: f32) {
    ...
    println!("  {} {}: {}{:.3} ({:.1}%)", 
            icon, metric_name, sign, delta, delta * 100.0);
}
...
fn generate_markdown_summary(
    ...
    md.push_str(&format!("**Validation Date:** {}\n", 
    ...
    md.push_str("|--------|-------|------------|\n");
    md.push_str(&format!("| Accuracy | {:.3} | {:.1}% |\n", 
                        result.overall_metrics.accuracy, result.overall_metrics.accuracy * 100.0));
    md.push_str(&format!("| Precision | {:.3} | {:.1}% |\n", 
                        result.overall_metrics.precision, result.overall_metrics.precision * 100.0));
    md.push_str(&format!("| Recall | {:.3} | {:.1}% |\n", 
                        result.overall_metrics.recall, result.overall_metrics.recall * 100.0));
    md.push_str(&format!("| F1-Score | {:.3} | {:.1}% |\n", 
                        result.overall_metrics.f1_score, result.overall_metrics.f1_score * 100.0));
    md.push_str(&format!("| False Positive Rate | {:.3} | {:.1}% |\n", 
                        result.overall_metrics.false_positive_rate, result.overall_metrics.false_positive_rate * 100.0));
    
    md.push_str("\n## Performance Benchmarks\n\n");
    md.push_str(&format!("- **Average Inference Time:** {:.2}ms\n", result.overall_metrics.avg_inference_time_ms));
    md.push_str(&format!("- **Maximum Inference Time:** {:.2}ms\n", result.overall_metrics.max_inference_time_ms));
    md.push_str(&format!("- **Minimum Inference Time:** {:.2}ms\n", result.overall_metrics.min_inference_time_ms));
    
    md.push_str("\n## Deployment Readiness\n\n");
    md.push_str(&format!("- **Ready for Production:** {}\n", 
                        if result.deployment_readiness.ready_for_production { "✅ Yes" } else { "❌ No" }));
    md.push_str(&format!("- **Readiness Score:** {:.1}/1.0\n", result.deployment_readiness.readiness_score));
...
fn generate_metrics_csv(result: &crate::ml::production_validation::ValidationResult) -> Result<String> {
    let mut csv = String::new();
    
    // Header
    csv.push_str("validator,accuracy,precision,recall,f1_score,false_positive_rate,avg_inference_time_ms\n");
    
    // Overall metrics
    csv.push_str(&format!("Overall,{:.6},{:.6},{:.6},{:.6},{:.6},{:.6}\n",
    ...
    for (validator_name, metrics) in &result.validator_results {
        csv.push_str(&format!("{},{:.6},{:.6},{:.6},{:.6},{:.6},{:.6}\n",
                             validator_name,
                             metrics.accuracy,
                             metrics.precision,
                             metrics.recall,
...
async fn generate_test_suites(args: &ModelValidationArgs) -> Result<()> {
    ...
    let findings = if let Some(input_file) = &args.findings_file {
        ...
        return Err(anyhow::anyhow!("--findings-file required for test suite generation"));
    };
    ...
    println!("📊 Processing {} findings for test suite generation", findings.len());
    ...
    save_test_suite(&security_suite, &output_dir.join("security_test_suite.json")).await?;
    save_test_suite(&performance_suite, &output_dir.join("performance_test_suite.json")).await?;
    save_test_suite(&edge_case_suite, &output_dir.join("edge_case_test_suite.json")).await?;
    save_test_suite(&regression_suite, &output_dir.join("regression_test_suite.json")).await?;
    
    println!("✅ Generated 4 test suites in: {}", output_dir.display());
    println!("  • security_test_suite.json ({} tests)", security_suite.test_cases.len());
    println!("  • performance_test_suite.json ({} tests)", performance_suite.test_cases.len());
    println!("  • edge_case_test_suite.json ({} tests)", edge_case_suite.test_cases.len());
    println!("  • regression_test_suite.json ({} tests)", regression_suite.test_cases.len());
...
fn generate_security_test_suite(findings: &[Finding]) -> Result<TestSuite> {
    let security_findings: Vec<_> = findings.iter()
        .filter(|f| f.analyzer.contains("security") || 
...
fn generate_performance_test_suite(findings: &[Finding]) -> Result<TestSuite> {
    ...
    let test_cases: Vec<_> = findings.iter()
        .take(sample_size)
        .enumerate()
        .map(|(i, finding)| TestCase {
            id: format!("perf_test_{}", i),
            finding: finding.clone(),
            expected_classification: matches!(finding.severity, Severity::Critical | Severity::High),
...
fn generate_edge_case_test_suite(findings: &[Finding]) -> Result<TestSuite> {
    let edge_cases: Vec<_> = findings.iter()
        .filter(|f| {
            // Identify edge cases
            f.message.len() < 10 || f.message.len() > 200 ||
            f.line == 1 || f.line > 1000 ||
            matches!(f.severity, Severity::Info) ||
            f.file.to_string_lossy().contains("test")
        })
        .collect();
    
    let test_cases: Vec<_> = edge_cases.iter()
        .enumerate()
        .map(|(i, finding)| TestCase {
