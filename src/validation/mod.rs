...
impl ValidationEngine {
    ...
    fn should_create_github_issue(&mut self, finding: &Finding) -> Result<bool> {
        ...
        if let Some(ml_classifier) = &mut self.ml_classifier {
            ...
            return Ok(matches!(finding.severity, Severity::Critical | Severity::High));
        }

        // Additional validation for secret detection
        if finding.rule.contains("secret") || finding.rule.contains("password") || finding.rule.contains("key") {
    ...
    fn is_likely_test_data(&self, finding: &Finding) -> bool {
        ...
        let test_indicators = [
            "test_", "example_", "sample_", "demo_", "mock_",
            "fake_", "dummy_", "placeholder_", "your_", "insert_",
        ...
        if finding.rule.contains("secret") || finding.rule.contains("password") || finding.rule.contains("key") {
            ...
            if content_to_check.contains("sk-") && (content_to_check.contains("1234") || content_to_check.contains("test") || content_to_check.contains("example")) {
        ...
        if file_content.contains("/test") || file_content.contains("\\test") || 
           file_content.contains("/example") || file_content.contains("\\example") {
            return test_indicators.iter().any(|indicator| file_content.contains(indicator));
        }
    ...
    fn validate_secret_finding(&self, finding: &Finding) -> bool {
        ...
        let test_patterns = [
            ...
            "placeholder", "xxx", "123", "abc", "password123", "secret123",
            "your_api_key", "your_secret", "insert_key_here", "replace_with",
        ];
    ...
    pub fn filter_results_for_github(&mut self, mut results: AnalysisResults) -> Result<AnalysisResults> {
        ...
        results.summary.metadata.insert(
            ...
            format!("{:.1}%", (1.0 - filtered_count as f64 / original_count.max(1) as f64) * 100.0)
        );
...
mod tests {
    ...
    fn test_test_data_detection() {
        ...
        if is_test_data {
            println!("Real finding incorrectly detected as test data: {}", real_finding.message);
        }
        assert!(!is_test_data);
    }
