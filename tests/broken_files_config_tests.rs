use codeguardian::config::{Config, BrokenFilesConfig, ConflictDetectionConfig, PlaceholderDetectionConfig, DuplicateDetectionConfig};
...
mod config_tests {
    ...
    fn test_config_serialization() {
        ...
        assert_eq!(config.detect_merge_conflicts, deserialized.detect_merge_conflicts);
        assert_eq!(config.detect_ai_placeholders, deserialized.detect_ai_placeholders);
...
"#;
        ...
        assert_eq!(config.placeholders.custom_patterns, vec!["my custom pattern"]);
...
"#;
        ...
        assert!(config.analyzers.broken_files.detect_ai_placeholders);
        assert!(config.analyzers.broken_files.conflicts.fail_on_conflicts);
        assert_eq!(config.analyzers.broken_files.placeholders.severity, "medium");
...
mod analyzer_registry_config_tests {
    ...
    fn test_analyzer_registry_with_disabled_broken_files() {
        ...
        let findings = registry.analyze_file(Path::new("test.rs"), test_content).unwrap();
...
"#;
        ...
        assert!(!has_ai_content, "Should not detect AI content when disabled");
    }
    ...
    fn test_analyzer_registry_with_custom_config() {
        ...
        let ai_findings = registry.analyze_file(Path::new("test.rs"), ai_content).unwrap();
...
"#;
        ...
        let dup_findings = registry.analyze_file(Path::new("test.rs"), duplicate_content).unwrap();
...
"#;
        ...
        let default_findings = default_registry.analyze_file(Path::new("test.rs"), test_content).unwrap();
        let custom_findings = custom_registry.analyze_file(Path::new("test.rs"), test_content).unwrap();
        ...
        assert!(!default_has_duplicates, "Default should not detect duplicates");
    ...
    fn test_config_validation_edge_cases() {
        ...
        config.analyzers.broken_files.duplicates.max_files_to_compare = usize::MAX;
        let registry = AnalyzerRegistry::with_config(&config);
        ...
        let test_content = b"fn test() {}";
        let _ = registry.analyze_file(Path::new("test.rs"), test_content).unwrap();
    }
}
...
"#;
        ...
        assert_eq!(config.analyzers.broken_files.placeholders.severity, "critical");
        assert_eq!(config.analyzers.broken_files.placeholders.custom_patterns.len(), 3);
        assert!(config.analyzers.broken_files.duplicates.focus_security);
        assert_eq!(config.analyzers.broken_files.duplicates.max_files_to_compare, 2000);
...
"#;
        ...
        assert_eq!(config.analyzers.broken_files.duplicates.max_files_to_compare, 1000);
    }
}