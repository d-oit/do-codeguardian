...
"##;
    ...
    let findings = analyzer
        .analyze(Path::new("test_file.rs"), test_content.as_bytes())
        .unwrap();
...
"#;
    ...
    let findings = analyzer
        .analyze(Path::new("main.rs"), conflict_content.as_bytes())
        .unwrap();
...
"#;
    ...
    let findings = analyzer
        .analyze(
            Path::new("tests/conflict_test.rs"),
    ...
    assert_eq!(
        findings.len(),
        0,
        "Should ignore conflict markers in test directory"
    );

    // Test with file that has "test" in filename
    let findings = analyzer
        .analyze(Path::new("test_conflicts.rs"), conflict_content.as_bytes())
        .unwrap();
    assert_eq!(
        findings.len(),
        0,
        "Should ignore conflict markers in test files"
    );
}
...
"#;
    ...
    let findings = analyzer
        .analyze(Path::new("documented.rs"), doc_content.as_bytes())
        .unwrap();
    ...
    for finding in incomplete_findings {
        let line_content = doc_content
            .lines()
            .nth((finding.line - 1) as usize)
            .unwrap_or("");
        assert!(
            !line_content.trim().starts_with("//"),
            "Should not flag TODO in documentation comments: {}",
            line_content
        );
        assert!(
            !line_content.trim().starts_with("///"),
            "Should not flag TODO in doc comments: {}",
            line_content
        );
    }
}
...
"#;
    ...
    let findings = analyzer
        .analyze(Path::new("mixed_content.rs"), test_content.as_bytes())
        .unwrap();
    ...
    for finding in incomplete_findings {
        let line_content = test_content
            .lines()
            .nth((finding.line - 1) as usize)
            .unwrap_or("");
        // Should not be from inside test module
        assert!(
            !line_content.contains("let content = \"TODO: implement this\""),
            "Should not flag TODO in test string literals"
        );
        assert!(
            !line_content.contains("let content2 = \"unimplemented!()\""),
...
"#;
    ...
    let findings = analyzer
        .analyze(Path::new(TEST_FILE), string_content.as_bytes())
        .unwrap();
    ...
    for finding in incomplete_findings {
        let line_content = string_content
            .lines()
            .nth((finding.line - 1) as usize)
            .unwrap_or("");
        ...
        assert!(
            !line_content.contains("\"NotImplementedException occurred\""),
...
"#;
    ...
    let findings = analyzer
        .analyze(Path::new("incomplete.rs "), incomplete_content.as_bytes())
        .unwrap();
    ...
    assert!(
        incomplete_findings.len() > 0,
        "Should detect incomplete implementations."
    );

    let has_unimplemented = incomplete_findings.iter().any(|f| {
        incomplete_content
            .lines()
            .nth((f.line - 1) as usize)
...
"#;
    ...
    let findings = analyzer
        .analyze(Path::new("malformed.rs "), malformed_content.as_bytes())
        .unwrap();
    ...
    assert!(findings
        .iter()
        .any(|f| f.rule == "malformed_conflict" || f.rule == "merge_conflict_start"));
}
...
"#,
 ...
 "#,
    ...
    assert!(
        ...
        "Analysis should complete in under 1 second for large test file "
    );
    assert_eq!(
        findings.len(),
        0,
        "Should not flag conflict markers in large test file "
    );
}
...
fn test_analyzer_memory_usage_stability() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..5 {
        ...
        let _ = git_analyzer
            .analyze(Path::new(&filename), test_content.as_bytes())
            .unwrap();
        let _ = ai_analyzer
            .analyze(Path::new(&filename), test_content.as_bytes())
            .unwrap();
