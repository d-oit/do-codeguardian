use codeguardian::cli::{Cli, Commands, CheckArgs};
...
"#;
        ...
        let conflict_findings: Vec<_> = results.findings.iter()
...
"#;
        ...
        let ai_findings: Vec<_> = results.findings.iter()
...
"#;
        ...
        let duplicate_findings: Vec<_> = results.findings.iter()
            .filter(|f| f.analyzer == "duplicate")
            .collect();
        
        if !duplicate_findings.is_empty() {
            assert!(duplicate_findings.iter().any(|f| f.rule_id == "internal_duplication"));
            
            // Security-relevant duplicates should have appropriate severity
            for finding in duplicate_findings {
                assert!(matches!(finding.severity, 
...
"#;
        ...
        let analyzers_found: std::collections::HashSet<_> = results.findings.iter()
            .map(|f| f.analyzer.as_str())
            .collect();
        
        assert!(analyzers_found.contains("git_conflict"), "Should detect git conflicts");
        assert!(analyzers_found.contains("ai_content"), "Should detect AI content");
...
"#;
        ...
        let has_conflicts = results.findings.iter().any(|f| {
            f.analyzer == "git_conflict" && matches!(f.severity, codeguardian::types::Severity::Critical)
        });
        
        assert!(has_conflicts, "Should detect critical conflicts for fail_on_conflicts test");
...
"#;
        ...
        let placeholder_findings: Vec<_> = results.findings.iter()
            .filter(|f| f.analyzer == "ai_content" && f.rule_id == "placeholder_content")
            .collect();
        
        assert!(!placeholder_findings.is_empty(), "Should detect custom patterns");
        assert!(placeholder_findings.len() >= 3, "Should detect all custom patterns");
    ...
    async fn test_performance_limits_integration() -> Result<(), Box<dyn std::error::Error>> {
        ...
        config.analyzers.broken_files.enabled = true;
        config.analyzers.broken_files.detect_duplicates = true;
        config.analyzers.broken_files.duplicates.max_files_to_compare = 10;
