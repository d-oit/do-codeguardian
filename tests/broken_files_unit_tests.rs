...
mod git_conflict_unit_tests {
    ...
    fn test_conflict_marker_patterns() {
        ...
        for (line, should_match) in test_cases {
            ...
            if should_match {
                assert!(!findings.is_empty(), "Should detect conflict marker: {}", line);
            } else {
                assert!(findings.is_empty(), "Should not detect false positive: {}", line);
            }
        }
    }
...
"#;
        ...
        let findings = analyzer.analyze(Path::new("test.toml"), invalid_toml.as_bytes()).unwrap();
...
"#;
        ...
        assert!(findings.iter().any(|f| f.rule_id == "suspicious_duplication"));
    ...
    fn test_large_file_with_conflicts() {
        ...
        let findings = analyzer.analyze(Path::new("large.rs"), content.as_bytes()).unwrap();
...
mod ai_content_unit_tests {
    ...
    fn test_ai_comment_markers() {
        ...
        for marker in test_cases {
            ...
            assert!(findings.iter().any(|f| f.rule_id == "ai_generated_marker"), 
                "Should detect AI marker: {}", marker);
        }
    }
    ...
    fn test_generic_function_patterns() {
        ...
        for func in generic_functions {
            ...
            assert!(findings.iter().any(|f| f.rule_id == "generic_function_name"), 
                "Should detect generic function: {}", func);
        }
    }
    ...
    fn test_incomplete_implementation_patterns() {
        ...
        for (pattern, expected_severity) in incomplete_patterns {
            ...
            let incomplete_finding = findings.iter()
                .find(|f| f.rule_id == "incomplete_implementation" || f.rule_id == "placeholder_content");
            
            assert!(incomplete_finding.is_some(), "Should detect incomplete: {}", pattern);
            
            if let Some(finding) = incomplete_finding {
                assert_eq!(finding.severity, expected_severity, 
                    "Wrong severity for: {}", pattern);
            }
        }
    }
...
"#;
        ...
        let findings = analyzer.analyze(Path::new("test.rs"), doc_content.as_bytes()).unwrap();
        
        // Should only detect the non-documentation TODO
        let placeholder_findings: Vec<_> = findings.iter()
...
"#;
        ...
        for path in test_paths {
            let findings = analyzer.analyze(Path::new(path), content.as_bytes()).unwrap();
            
            // Generic function names should be ignored in test/example files
            assert!(!findings.iter().any(|f| f.rule_id == "generic_function_name"), 
                "Should ignore generic functions in: {}", path);
        }
    }
...
"#;
        ...
        assert_eq!(findings.iter().filter(|f| f.rule_id == "placeholder_content").count(), 3);
    }
    ...
    fn test_file_type_filtering() {
        ...
        let supported_files = vec![
            "test.rs", "test.js", "test.ts", "test.py", "test.java",
            "test.cpp", "test.go", "test.php", "test.rb", "test.cs",
        ];
        
        for file in supported_files {
            assert!(analyzer.supports_file(Path::new(file)), 
                "Should support: {}", file);
        }
        
        // Should not support non-source files
        let unsupported_files = vec![
            "test.txt", "test.md", "test.pdf", "test.exe", "test.png",
        ];
        
        for file in unsupported_files {
            assert!(!analyzer.supports_file(Path::new(file)), 
                "Should not support: {}", file);
        }
    }
}
...
"#, func_name, func_name);
            ...
            let findings = analyzer.analyze(Path::new("security.rs"), content.as_bytes()).unwrap();
...
"#;
        ...
        let findings = analyzer.analyze(Path::new("utils.rs"), non_security_content.as_bytes()).unwrap();
...
"#;
            ...
            if should_detect {
                assert!(!findings.is_empty(), "Should detect with min_lines={}", min_lines);
            } else {
                assert!(findings.is_empty(), "Should not detect with min_lines={}", min_lines);
            }
        }
    }
...
"#;
        ...
        let findings_include = analyzer_include_tests.analyze(test_file_path, test_content.as_bytes()).unwrap();
        let findings_exclude = analyzer_exclude_tests.analyze(test_file_path, test_content.as_bytes()).unwrap();
...
"#, false, "Different code"),
        ...
        for (content, should_detect, description) in test_cases {
            ...
            if should_detect {
                assert!(!findings.is_empty(), "Should detect duplication: {}", description);
                if !findings.is_empty() {
                    assert!(findings[0].message.contains("%"), 
                        "Should include similarity percentage: {}", description);
                }
            } else {
                assert!(findings.is_empty(), "Should not detect duplication: {}", description);
            }
        }
    }
...
"#;
        ...
        for path in generated_paths {
            ...
            assert!(findings.is_empty(), "Should ignore generated file: {}", path);
        }
    }
...
"#;
        ...
        let findings = analyzer.analyze(Path::new("test.rs"), content_with_blocks.as_bytes()).unwrap();
...
"#;
        ...
        let findings = analyzer.analyze(Path::new("test.rs"), content_with_whitespace.as_bytes()).unwrap();
    ...
    fn test_supported_file_types() {
        ...
        let unsupported_extensions = vec![
            "txt", "md", "json", "xml", "html", "css", "png", "jpg"
        ];
...
"#;
        ...
        let high_findings = analyzer.analyze(Path::new("crypto.rs"), high_risk_content.as_bytes()).unwrap();
        let medium_findings = analyzer.analyze(Path::new("validation.rs"), medium_risk_content.as_bytes()).unwrap();
        ...
        if !medium_findings.is_empty() {
            assert!(medium_findings.iter().any(|f| 
