...
mod performance_tests {
    ...
    fn test_git_conflict_analyzer_performance() {
        ...
        let findings = analyzer.analyze(Path::new("small.rs"), small_content.as_bytes()).unwrap();
        let duration = start.elapsed();
        
        assert!(duration.as_millis() < SMALL_FILE_THRESHOLD_MS, 
            "Small file analysis took too long: {:?}", duration);
        assert!(!findings.is_empty(), "Should detect conflicts in generated content");
        
        // Large file performance
        let large_content = generate_file_with_conflicts(10000);
        let start = Instant::now();
        let findings = analyzer.analyze(Path::new("large.rs"), large_content.as_bytes()).unwrap();
        let duration = start.elapsed();
        
        assert!(duration.as_millis() < PERFORMANCE_THRESHOLD_MS, 
            "Large file analysis took too long: {:?}", duration);
        assert!(!findings.is_empty(), "Should detect conflicts in large content");
    }
    ...
    fn test_ai_content_analyzer_performance() {
        ...
        assert!(duration.as_millis() < SMALL_FILE_THRESHOLD_MS, 
            "Small file analysis took too long: {:?}", duration);
        assert!(!findings.is_empty(), "Should detect AI content in generated content");
    ...
    fn test_duplicate_analyzer_performance() {
        ...
        let start = Instant::now();
        let findings = analyzer.analyze(Path::new("medium.rs"), medium_content.as_bytes()).unwrap();
        let duration = start.elapsed();
        
        assert!(duration.as_millis() < PERFORMANCE_THRESHOLD_MS, 
            "Medium file analysis took too long: {:?}", duration);
    }
    ...
    fn test_memory_usage_large_files() {
        ...
        for analyzer in analyzers {
            ...
            assert!(result.is_ok(), "Analyzer {} failed on large file", analyzer.name());
    ...
    fn test_parallel_analysis_performance() {
        ...
        let handles: Vec<_> = (0..4).map(|i| {
            ...
            thread::spawn(move || {
                ...
                analyzer.analyze(Path::new(&path), content.as_bytes()).unwrap()
            })
        }).collect();
        ...
        assert!(duration.as_millis() < PERFORMANCE_THRESHOLD_MS, 
            "Parallel analysis took too long: {:?}", duration);
    ...
    fn test_regex_compilation_performance() {
        ...
        assert!(git_duration.as_millis() < 100, 
            "Git analyzer creation too slow: {:?}", git_duration);
        assert!(ai_duration.as_millis() < 200, 
            "AI analyzer creation too slow: {:?}", ai_duration);
        assert!(dup_duration.as_millis() < 100, 
            "Duplicate analyzer creation too slow: {:?}", dup_duration);
    }
    ...
    fn test_worst_case_scenarios() {
        ...
        let analyzer = GitConflictAnalyzer::new();
        let start = Instant::now();
        let findings = analyzer.analyze(Path::new("conflicts.rs"), many_conflicts.as_bytes()).unwrap();
        let duration = start.elapsed();
        
        assert!(duration.as_millis() < PERFORMANCE_THRESHOLD_MS, 
            "Many conflicts analysis took too long: {:?}", duration);
        ...
        let analyzer = AiContentAnalyzer::new();
        let start = Instant::now();
        let findings = analyzer.analyze(Path::new("placeholders.rs"), many_placeholders.as_bytes()).unwrap();
        let duration = start.elapsed();
        
        assert!(duration.as_millis() < PERFORMANCE_THRESHOLD_MS, 
            "Many placeholders analysis took too long: {:?}", duration);
        ...
        let start = Instant::now();
        let findings = analyzer.analyze(Path::new("duplicates.rs"), many_duplicates.as_bytes()).unwrap();
        let duration = start.elapsed();
        
        assert!(duration.as_millis() < PERFORMANCE_THRESHOLD_MS * 2, // Allow more time for duplicates
            "Many duplicates analysis took too long: {:?}", duration);
    }
    ...
    fn test_incremental_analysis_performance() {
        ...
        for i in 0..iterations {
            ...
            let start = Instant::now();
            let _findings = analyzer.analyze(Path::new("test.rs"), modified_content.as_bytes()).unwrap();
            total_duration += start.elapsed();
        }
        
        let avg_duration = total_duration / iterations;
        assert!(avg_duration.as_millis() < SMALL_FILE_THRESHOLD_MS, 
            "Average incremental analysis too slow: {:?}", avg_duration);
    }
    ...
    fn test_file_type_filtering_performance() {
        ...
        assert!(duration.as_millis() < 50, 
            "File type filtering too slow: {:?}", duration);
    }
...
mod benchmark_tests {
    ...
    fn benchmark_analyzer_comparison() {
        ...
        for (analyzer, name) in analyzers {
            let start = Instant::now();
            let findings = analyzer.analyze(Path::new("benchmark.rs"), test_content.as_bytes()).unwrap();
            let duration = start.elapsed();
            
            results.push((name, duration, findings.len()));
            
            println!("Analyzer {}: {:?} ({} findings)", name, duration, findings.len());
        }
        
        // All analyzers should complete within reasonable time
        for (name, duration, _) in &results {
            assert!(duration.as_millis() < PERFORMANCE_THRESHOLD_MS, 
                "Analyzer {} exceeded performance threshold: {:?}", name, duration);
        }
    }
    ...
    fn benchmark_scaling_behavior() {
        ...
        for size in sizes {
            ...
            let start = Instant::now();
            let _findings = analyzer.analyze(Path::new("test.rs"), content.as_bytes()).unwrap();
            let duration = start.elapsed();
            ...
            if prev_duration.as_millis() > 0 {
                ...
                assert!(ratio < 10.0, "Performance scaling too poor: {}x slower", ratio);
            }
